...
- On every boot the worker registers its build (deployment `TEMPORAL_DEPLOYMENT_NAME`, build id = `git rev-parse --short=6 HEAD`) and **auto-promotes it to the deployment's Current Version**; re-deploying an older SHA re-promotes it (that's the rollback story). Workflows default to AUTO_UPGRADE. **Never deploy from a dirty tree** — the build id is the git SHA, so a dirty deploy replays a different bundle under the previous version id.
