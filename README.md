...
2. **Ticket attribute**: `/intercom → Bridge → Ensure Ticket Attributes` — creates the `SLA Target` **ticket** attribute on every mapped ticket type (the bot mirrors each target onto the converted ticket too; the API *can* create ticket attributes).
3. **Bridged-tickets Workflow**: trigger **Teammate adds a note**, context **Customer tickets ONLY** — ticket contexts have no channel gate, and API-created conversations never match a channel, so a conversation-scoped trigger would never fire for bridged tickets (there is also no attribute-change trigger). One branch per `SLA Target` **ticket attribute** value → native **Apply SLA**.
4. **Native-conversations Workflow** (only with *Native* enabled): same trigger, context **Conversations**, ALL channels — native conversations come from real channels. Branch on the **conversation** attribute → Apply SLA. The two Workflows cover disjoint subjects, so SLAs never double-apply.
5. **Note kick**: the bot posts a small internal note on every target change (toggleable in the SLA hub) — it lands AFTER the attribute writes, so branches read fresh values. **Test once**: flip a target on a test ticket and confirm the Workflow applied the SLA; if the trigger ignores bot-authored notes in your workspace, SLAs still converge on the first human agent note/reply.
6. **Webhooks** (native-conversation rules only): Developer Hub → your app → Webhooks → subscribe `conversation.user.created` + `conversation.user.replied`. Bridged tickets work without them.
7. Toggle **SLA: on** (and **Native** if wanted) in `/intercom → SLA Manager`.
