...
export class FraudHuntService {
  ...
  // exact-match id the fingerprint hunt takes.
  async cardsByLast4(last4: string, brand?: string): Promise<HuntResult<Last4Group>> {
    ...
    const { charges, nextPage } = await this.stripe.searchChargesByCardLast4(last4, cleanBrand || undefined, 100);
  ...
  // Amount search hits PaymentIntents, so it surfaces DECLINED / blocked
  // attempts (issuer-refused renewals) that never became a charge — exactly
  // what the last4/charge searches cannot see. amountMajor is parsed here so
  // both surfaces share the zero-decimal handling.
