...
// Automation hub (intercom group): the workspace inactivity sweeper for native /
// unbridged conversations + tickets. Mirrors /intercom → Automation.
// (Per-tag reminder TEXT overrides are edited on each tag in Workflow.)
  ...
  return {
    ...
    async buildSections(ctx): Promise<Section[]> {
      ...
      return [
        {
          key: "inactivity",
          title: "Inactivity sweeper",
          fields: [
            { type: "toggle", key: "inactivityEnabled", label: "Enabled", value: s.inactivityEnabled() },
            { type: "number", key: "inactivityAgentWaitDays", label: "Agent-idle days before nudge", value: s.inactivityAgentWaitDays(), min: 1, max: 30 },
            ...
            { type: "text", key: "inactivityNagText", label: "Customer nudge text", value: s.inactivityNagText() ?? "", multiline: true, help: "Blank = built-in default." },
            { type: "text", key: "inactivityAgentNoteText", label: "Agent-idle note text", value: s.inactivityAgentNoteText() ?? "", multiline: true, help: "Blank = built-in default." },
          ],
          actions: [{ key: "run_inactivity", label: "Run inactivity sweep now", style: "secondary" }],
        },
      ];
    },
    ...
    async save(ctx: AdminHubContext, req: SaveRequest): Promise<SaveResult> {
      ...
      switch (req.field) {
        ...
        case "inactivityCustomerWaitDays": {
          ...
          await s.updateInactivity({ [req.field]: parsed.value });
        ...
        case "inactivityAgentNoteText":
          await s.updateInactivity({ [req.field]: asString(v) || null });
